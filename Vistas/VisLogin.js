...
const VisLogin = (props) => {
  ...
  useEffect(() => {
    const unsubscribe = auth.onAuthStateChanged(user => {
      if (user) {
        props.navigation.navigate("VisMenuDrawer")
      }
    })
  ...
  const handleGoogleSigingIn= () => {
    const config= {
      webClientId: '737205349718-cjple3t8l7qt6u1j5inq9je3rb57ibn5.apps.googleusercontent.com',
      ...
      .then((result)=>{
        ...
        if(type=='success'){
          ...
          setTimeout(()=>props.navigate('VInicio',{email,name,photoUrl}),1000)
           ...
           navigation.replace("VisMenuDrawer")
        }else{
